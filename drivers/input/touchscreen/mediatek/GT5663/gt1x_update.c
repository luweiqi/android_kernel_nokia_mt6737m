...
#define GT1X_FW_NAME      "goodix_update.bin"
#endif

//#define UPDATE_FILE_PATH_1          "/data/tp_firmware/_goodix_update_.bin"
//#define UPDATE_FILE_PATH_2          "/sdcard/tp_firmware/_goodix_update_.bin"
//#define CONFIG_FILE_PATH_1          "/data/tp_firmware/_gt1x_config_.cfg"
//#define CONFIG_FILE_PATH_2          "/sdcard/tp_firmware/_gt1x_config_.cfg"

#define UPDATE_FILE_PATH_1          "/vendor/firmware/goodix_update.bin"
#define UPDATE_FILE_PATH_2          "/sdcard/firmware/goodix_update.bin"

#define CONFIG_FILE_PATH_1_0        "/vendor/firmware/goodix_config_0.cfg"
#define CONFIG_FILE_PATH_2_0        "/sdcard/firmware/goodix_config_0.cfg"
#define CONFIG_FILE_PATH_1_1        "/vendor/firmware/goodix_config_1.cfg"
#define CONFIG_FILE_PATH_2_1        "/sdcard/firmware/goodix_config_1.cfg"
...
char *config_path_vendor = NULL;
char *config_path_sdcard = NULL;
...
int gt1x_auto_update_proc(void *data)
{
	...
	if(ret == 0){
		GTP_ERROR("Invalid Tp sendor ID, update failed!!!");
		return 0;
	}
	...
	if (ret & (FOUND_CFG_PATH_1 | FOUND_CFG_PATH_2)) {
		if (ret & FOUND_CFG_PATH_1) {
			filename = config_path_vendor;
		} else {
			filename = config_path_sdcard;
		}
		...
		if (gt1x_parse_config(filename, config) > 0) {
			...
			if(ret < 0){
				GTP_ERROR("firefly read config version failed!");
				return 0;
			}
			GTP_INFO("firefly IC current version = 0x%02x", config_version);
			GTP_INFO("firefly update config version = 0x%02x", config[0]);
			if(config[0] <= config_version){
				GTP_ERROR("firefly Update config version <= ic current version");
				GTP_ERROR("firefly Update config failed!");
				return 0;
			}
...
static int gt1x_search_update_files(void)
{
	int retry = 20 * 3;	//wait 30s(max) if fs is not ready
	struct file *pfile = NULL;
	mm_segment_t old_fs;
	int found = 0;
	int id = gt1x_version.sensor_id;
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	...
	while (retry-- > 0) {
		...
		if (gt1x_check_fs_mounted("/vendor")) {
			GTP_DEBUG("vendor filesystem is not ready");
			continue;
		}
		// search firmware
		...
		// search config file
		GTP_INFO("firefly: Update get tp sendor id = %d\n", id);
		switch(id){
			case 0:
				config_path_vendor = CONFIG_FILE_PATH_1_0;
				config_path_sdcard = CONFIG_FILE_PATH_2_0;
				break;
			case 1:
				config_path_vendor = CONFIG_FILE_PATH_1_1;
				config_path_sdcard = CONFIG_FILE_PATH_2_1;
				break;
			default:
				GTP_ERROR("firefly Tp sensor id invalid");
				return 0;
				break;

		}
		pfile = filp_open(config_path_vendor, O_RDONLY, 0);
		if (IS_ERR(pfile)) {
			pfile = filp_open(config_path_sdcard, O_RDONLY, 0);
			if (!IS_ERR(pfile)) {
...
int gt1x_update_firmware(void *filename)
{
	...
	for (i = 1; i < update_info.firmware_info->subsystem_count; i++) {
		GTP_DEBUG("subsystem: %d", update_info.firmware_info->subsystem[i].type);
		GTP_DEBUG("Length: %d", update_info.firmware_info->subsystem[i].length);
		GTP_DEBUG("Address: %d", update_info.firmware_info->subsystem[i].address);
...
int gt1x_update_prepare(char *filename)
{
	...
	if (filename == NULL) {
		...
		GTP_INFO("Firmware size: %d", (int)update_info.fw->size);
...
int gt1x_check_firmware(void)
{
	...
	GTP_DEBUG("Firmware product: GT%s", update_info.firmware_info->pid);
	GTP_DEBUG("Firmware patch: %02X%02X%02X", update_info.firmware_info->version[0], update_info.firmware_info->version[1], update_info.firmware_info->version[2]);
	GTP_DEBUG("Firmware chip: 0x%02X", update_info.firmware_info->chip_type);
	GTP_DEBUG("Subsystem count: %d", update_info.firmware_info->subsystem_count);
